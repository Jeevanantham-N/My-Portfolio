    ...
    <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-gray-100/50">
      <nav className="max-w-7xl mx-auto px-8 py-4 flex items-center justify-between text-base font-medium">
        ...
        <div className="flex gap-8">
          ...
          <a href="#contact" className="px-4 py-1.5 rounded-full bg-black text-white hover:opacity-80 transition-opacity">Contact</a>
        </div>
      </nav>
    </header>
  )
    ...
    <section className="max-w-7xl mx-auto px-8 py-32">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
        <div className="space-y-8">
          ...
          <p className="text-2xl text-gray-800">{data.role}</p>
          <p className="text-lg text-gray-600 max-w-2xl leading-relaxed">{data.summary}</p>
          <div className="flex gap-4">
            <a href="#contact" className="px-6 py-3 rounded-full border border-gray-300 text-base hover:shadow-md transition-shadow">Contact</a>
            <a href="#projects" className="px-6 py-3 rounded-full bg-black text-white text-base hover:opacity-90 transition-opacity">View Projects</a>
            <a href={resumePdf} download className="px-6 py-3 rounded-full border border-gray-300 text-base hover:shadow-md transition-shadow">Download CV</a>
          </div>
        ...
        <div className="hidden lg:block sticky top-32">
          <div className="p-10 rounded-3xl bg-white shadow-lg border border-gray-100/50">
            <div className="text-sm text-gray-500 uppercase tracking-wide">Role</div>
            <div className="mt-2 text-2xl font-bold">{data.role}</div>
            <div className="mt-6 text-base text-gray-600 leading-relaxed">Certified Adobe Analytics Developer & AEP practitioner. Focused on tracking, schema design, data validation, and migration.</div>
            <div className="mt-8">
              <a href="#contact" className="px-5 py-2.5 rounded-full border border-gray-300 text-base hover:shadow-md transition-shadow">Email</a>
            </div>
          </div>
        </div>
      </div>
...
function SectionTitle({ children }) {
  return <h2 className="text-3xl font-bold tracking-tight mb-8">{children}</h2>
}
    ...
    <section id="experience" className="max-w-7xl mx-auto px-8 py-16">
      ...
      <div className="space-y-8">
        {experiences.map((exp, idx) => (
          <article key={idx} className="p-8 rounded-3xl bg-white shadow-md hover:shadow-xl transition-shadow duration-300">
            <div className="flex flex-col md:flex-row items-start justify-between gap-4">
              <div>
                <div className="text-xl font-semibold">{exp.title}</div>
                <div className="text-base text-gray-500">{exp.company} • {exp.location}</div>
              </div>
              <div className="text-base text-gray-400">{exp.period}</div>
            </div>
            <ul className="mt-6 space-y-4 text-gray-600 text-base leading-relaxed list-disc pl-5">
        ...
        {education.map((edu, idx) => (
          <article key={idx} className="p-8 rounded-3xl bg-white shadow-md hover:shadow-xl transition-shadow duration-300">
            <div className="flex flex-col md:flex-row items-start justify-between gap-4">
              ...
              <div className="text-base text-gray-400">{edu.period}</div>
            </div>
          </article>
        ))}
      </div>
  ...
  return (
    <section id="projects" className="max-w-7xl mx-auto px-8 py-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {projects.map((p, i) => (
          <article key={i} className="p-8 rounded-3xl bg-white shadow-md hover:shadow-xl transition-shadow duration-300">
            <div className="flex flex-col md:flex-row items-start justify-between gap-4">
              <h3 className="text-xl font-semibold">{p.title}</h3>
              <div className="text-base text-gray-400">{p.tech.join(' • ')}</div>
            </div>
            <p className="mt-4 text-gray-600 text-base leading-relaxed">{p.desc}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
        <div>
          ...
          <div className="flex flex-wrap gap-3">
            {tools.map((t, i) => (
              <span key={i} className="px-4 py-2 bg-gray-100 rounded-full text-base font-medium text-gray-800">{t}</span>
            ))}
          </div>
        </div>
        <div>
...
function Toast({ message }) {
  ...
  return <div className="fixed bottom-8 right-8 bg-gray-900 text-white px-6 py-3 rounded-full shadow-lg">{message}</div>
}
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-50">
      <div className="bg-white p-8 rounded-3xl max-w-md w-full shadow-2xl">
        ...
        <p className="text-gray-600 mb-6">Click to open your email app, or copy the text below.</p>
        ...
        <button onClick={() => onCopy(decodeURIComponent(body))} className="block w-full mb-4 px-6 py-3 border border-gray-300 rounded-full hover:shadow-md transition-shadow">Copy Message</button>
        <button onClick={onClose} className="block w-full px-6 py-3 text-gray-600 rounded-full hover:bg-gray-100 transition-colors">Close</button>
      </div>
    </div>
  )
}
...
function Contact({ data }) {
  ...
  return (
    <section id="contact" className="max-w-7xl mx-auto px-8 py-16">
      ...
      <div className="p-8 rounded-3xl bg-white shadow-md max-w-2xl">
        ...
        <form ref={form} className="space-y-6" onSubmit={sendEmail}>
          <div>
            <label className="block text-base text-gray-700 mb-2" htmlFor="name">Your name</label>
            <input id="name" className="w-full p-4 border border-gray-300 rounded-2xl focus:outline-none focus:border-black transition-colors" value={name} onChange={(e) => { setName(e.target.value); setValidationError(null); setToast(null) }} />
          </div>
          <div>
            <label className="block text-base text-gray-700 mb-2" htmlFor="email">Your email</label>
            <input id="email" type="email" className="w-full p-4 border border-gray-300 rounded-2xl focus:outline-none focus:border-black transition-colors" value={userEmail} onChange={(e) => { setUserEmail(e.target.value); setValidationError(null); setToast(null) }} />
          </div>
          <div>
            <label className="block text-base text-gray-700 mb-2" htmlFor="subject">Subject</label>
            <input id="subject" className="w-full p-4 border border-gray-300 rounded-2xl focus:outline-none focus:border-black transition-colors" value={subject} onChange={(e) => setSubject(e.target.value)} />
          </div>
          <div>
            <label className="block text-base text-gray-700 mb-2" htmlFor="msg">Message</label>
            <textarea id="msg" className="w-full p-4 border border-gray-300 rounded-2xl focus:outline-none focus:border-black transition-colors" rows={6} value={message} onChange={(e) => { setMessage(e.target.value); setValidationError(null); setToast(null) }} />
          </div>
          {validationError && <div className="text-base text-red-600">{validationError}</div>}
          <div className="flex items-center gap-4">
            <button type="submit" disabled={isSending} className={`px-6 py-3 rounded-full text-base shadow-md transition-opacity ${isSending ? 'bg-gray-200 text-gray-500' : 'bg-black text-white hover:opacity-90'}`}>
              {isSending ? `Sending${attempts ? ` (attempt ${attempts})` : '...'}` : 'Send message'}
            </button>
            <button type="button" className="px-6 py-3 rounded-full bg-gray-100 text-base hover:shadow-md transition-shadow" onClick={() => {
            ...
            <div className="text-base text-gray-500 ml-auto">Secure via EmailJS</div>
          </div>
        </form>
...
function Footer({ data }) {
  return (
    <footer className="border-t border-gray-100/50">
